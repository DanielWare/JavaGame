 ...
 */
 private int getIndex(GameFigure pRect) {
   ...
   boolean topQuadrant = (pRect.getCollisionBox().getY() < horizontalMidpoint && pRect.getCollisionBox().getY() + pRect.getCollisionBox().getHeight() < horizontalMidpoint);
   // Object can completely fit within the bottom quadrants
   boolean bottomQuadrant = (pRect.getCollisionBox().getY() > horizontalMidpoint);
 
   // Object can completely fit within the left quadrants
   if (pRect.getCollisionBox().getX() < verticalMidpoint && pRect.getCollisionBox().getX() + pRect.getCollisionBox().getWidth() < verticalMidpoint) {
      if (topQuadrant) {
        index = 1;
      }
    ...
    else if (pRect.getCollisionBox().getX() > verticalMidpoint) {
     if (topQuadrant) {
       index = 0;
     }
     else if (bottomQuadrant) {
 ...
 */
 public List retrieve(List<GameFigure> returnObjects, GameFigure pRect) {
   int index = getIndex(pRect);
